...
//Parámetros lectura voltaje:
//Voltaje Despues del filtro:
float lecturaAnalogica =0; //Lectura analogica motor.
float voltajeSalida = 0; //Voltaje Generador.
//Voltaje Antes del filtro:
float lecturaAnalogica2 =0; //Lectura analogica motor.
float voltajeAnterFiltro = 0; //Voltaje Generador.
...
double pi=3.141592653589793238462643383279502884197169399375; //Constante Pi
...
//INICIO DEL CODIGO:-------------------------------------------------------------------------------------------------------------------------
//Funcion del encoder:
void encoderHVoid(){ //Esta parte del codigo entra a funcionar cuando sucede una interrupcion en el encoder
...
void setup() {
  ...
  pinMode(encoder, INPUT_PULLUP);  //Se necesita el pin del encoder como entrada para la interrupcion.
  pinMode(ENA, OUTPUT);
  ...
  attachInterrupt(digitalPinToInterrupt(encoder),encoderHVoid,RISING);
}
...
void loop() {
  ...
  if(millis() - timeold >= PeriodoMuestreo){  //Se obtendrá las muestras de velocidad, voltaje y PWM para los instantes de tiempo correspondientes al tiempo de muestreo.
    //Salida de monitor Serial para imprimir los datos:
    Serial.print(" "); Serial.print(w); Serial.print(" ");  //Velocidad en radianes por segundo.
